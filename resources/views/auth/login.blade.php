<x-guest-layout>
    <style>
        /* Dark animated gradient behind everything */
    ...
    {{-- GAME CANVAS (behind content, above gradient) --}}
                ...
                <form method="POST" action="{{ route('login') }}" class="space-y-5">
                    ...
                    <div>
                        ...
                        <x-text-input
                            ...
                            placeholder="ievads@example.com"
                            class="mt-1 block w-full bg-gray-800 border border-gray-700 text-gray-100 placeholder-gray-300 rounded-lg
                    ...
                    <div class="flex items-center justify-between pt-1">
                        <label for="remember_me" class="inline-flex items-center gap-2 select-none">
                            ...
                            <span class="text-sm text-gray-300">{{ __('Atcerēties mani') }}</span>
                        </label>

                        @if (Route::has('password.request'))
                            <a class="text-sm underline text-gray-300 hover:text-emerald-300 transition"
                    ...
                    <div class="flex flex-col sm:flex-row items-center justify-between gap-3 pt-2">
                        <a class="text-sm underline text-gray-300 hover:text-emerald-300 transition" href="{{ route('register') }}">
                            {{ __('Nav konta? Reģistrējies') }}
                        </a>

                        <x-primary-button
                            class="px-6 py-2.5 bg-emerald-600 hover:bg-emerald-700 rounded-lg font-semibold
                                   shadow-lg shadow-emerald-900/30 focus:ring-emerald-500 transition">
                            {{ __('Pieteikties') }}
                        </x-primary-button>
                    </div>
                </form>
    ...
    (() => {
        ...
        function drawTarget(x, y, r){
            const colors=["red","white","black","white","red"]; const w=r/colors.length;
            for (let i=colors.length-1;i>=0;i--){ ctx.beginPath(); ctx.arc(x,y,w*(i+1),0,Math.PI*2); ctx.fillStyle=colors[i]; ctx.fill(); }
        }
        function drawCrosshair(x,y){
            ctx.strokeStyle="white"; ctx.lineWidth=2; ctx.beginPath();
            ctx.moveTo(x-15,y); ctx.lineTo(x+15,y); ctx.moveTo(x,y-15); ctx.lineTo(x,y+15); ctx.stroke();
        }
        class Target{ constructor(x,y,r,s){ this.x=x; this.y=y; this.r=r; this.s=s; }
        ...
        class Bullet{ constructor(x,y){ this.x=x; this.y=y; this.r=5; this.s=7; }
            update(){ this.y-=this.s; this.draw(); }
            draw(){ ctx.beginPath(); ctx.arc(this.x,this.y,this.r,0,Math.PI*2); ctx.fillStyle="yellow"; ctx.fill(); } }

        for (let i=0;i<6;i++){ targets.push(new Target(Math.random()*canvas.width, Math.random()*canvas.height, 20, 1+Math.random()*2)); }
        ...
        const animate = () => {
            ...
            for (let i=targets.length-1;i>=0;i--){ const t=targets[i];
                for (let j=bullets.length-1;j>=0;j--){ const b=bullets[j]; const dx=b.x-t.x, dy=b.y-t.y;
                    if (dx*dx+dy*dy < t.r*t.r){ targets.splice(i,1); bullets.splice(j,1);
                        targets.push(new Target(Math.random()*canvas.width,-20,20,1+Math.random()*2)); break; } } }
