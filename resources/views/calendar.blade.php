<x-app-layout>
    <div class="max-w-7xl mx-auto p-6">
        <h1 class="text-3xl font-bold text-gray-200 mb-6">Medību kalendārs Latvijā</h1>
        ...
        <div class="flex space-x-6 overflow-x-auto pb-6">
            @foreach($months as $month)
                @php
                    ...
                    $firstDayOfWeek = $date->dayOfWeek; // 0 = Sunday
                @endphp

                <div class="flex-none w-96 bg-gray-800 rounded-lg shadow-lg p-4 text-gray-200">
                    <h2 class="text-xl font-semibold mb-4 text-center">{{ $date->locale('lv')->isoFormat('MMMM YYYY') }}</h2>
                    ...
                    <div class="mt-4">
                        ...
                        <div class="flex flex-wrap gap-2">
                            @foreach($animals as $animal)
                                ...
                                @endif
                            @endforeach
                            @if(!collect($animals)->pluck('months')->flatten()->contains($month))
