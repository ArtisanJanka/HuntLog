    ...
    <section class="relative min-h-screen pt-20 sm:pt-24 overflow-hidden">
        ...
        <div class="relative z-10 max-w-2xl mx-auto px-4 sm:px-6 lg:px-8 py-6 sm:py-10" data-reveal-group>
            ...
            <header class="reveal mb-6">
                <h1 class="text-3xl sm:text-4xl font-black tracking-tight text-white">Pievienot attēlu</h1>
                <p class="mt-2 text-gray-300">Izvēlies medību tipu, pievieno nosaukumu (nav obligāts) un augšupielādē attēlu.</p>
            </header>

            {{-- Forma --}}
            <section class="reveal rounded-2xl border border-white/10 bg-white/5 backdrop-blur-xl shadow-2xl p-6 sm:p-8">
                <form method="POST" action="{{ route('admin.gallery.store') }}" enctype="multipart/form-data" class="space-y-7" id="galleryCreateForm">
                    @csrf
                    ...
                    {{-- Attēls (drag & drop + priekšskatījums) --}}
                    <div>
                        <label class="block text-sm font-semibold text-white mb-1.5">Attēls <span class="text-gray-400 font-normal">(obligāts)</span></label>
                        ...
                        <div id="dropzone"
                            ...
                            <input id="image" type="file" name="image" accept="image/*" required class="absolute inset-0 opacity-0 cursor-pointer" />
                        ...
                        {{-- Priekšskatījums jaunajam failam --}}
