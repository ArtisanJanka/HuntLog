<x-app-layout>
    <div class="max-w-xl mx-auto p-6">
        <h1 class="text-2xl font-semibold text-black mb-4">Add Image</h1>
        <form method="POST" action="{{ route('admin.gallery.store') }}" enctype="multipart/form-data" class="space-y-4">
            @csrf
            <div>
                <label class="block text-black-300 mb-1">Hunting Type</label>
            ...
            <div>
                <label class="block text-black-300 mb-1">Title (optional)</label>
                <input name="title" value="{{ old('title') }}" class="w-full bg-gray-800 text-white rounded p-2">
                @error('title') <p class="text-red-400 text-sm mt-1">{{ $message }}</p> @enderror
            </div>
            <div>
                <label class="block text-black-300 mb-1">Image</label>
