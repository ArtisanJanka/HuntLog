<x-app-layout>
    ...
    <section class="relative min-h-screen pt-20 sm:pt-24 overflow-hidden">
        ...
        <div class="relative z-10 max-w-2xl mx-auto px-4 sm:px-6 lg:px-8 py-6 sm:py-10" data-reveal-group>
            ...
            <header class="reveal mb-6">
                <h1 class="text-3xl sm:text-4xl font-black tracking-tight text-white">Rediģēt medību tipu</h1>
                <p class="mt-2 text-gray-300">Atjauno nosaukumu un (ja vajag) savu <span class="text-emerald-300">slug</span>.</p>
            </header>

            {{-- Forma --}}
            <section class="reveal rounded-2xl border border-white/10 bg-white/5 backdrop-blur-xl shadow-2xl p-6 sm:p-8">
                <form method="POST" action="{{ route('admin.hunting-types.update', $type) }}" class="space-y-6" id="typeForm">
                    @csrf
                    @method('PUT')
                    ...
                    <div>
                        ...
                        <input id="name" name="name" value="{{ old('name', $type->name) }}"
                    ...
                    <div>
                        <div class="flex items-center justify-between gap-3">
                            ...
                            <label class="flex items-center gap-2 text-sm text-gray-300 select-none">
                                <input id="autoSlug" type="checkbox" class="rounded border-gray-600 text-emerald-600 focus:ring-emerald-500">
                                Automātiski ģenerēt
                            </label>
                        </div>

                        <input id="slug" name="slug" value="{{ old('slug', $type->slug) }}"
    ...
    {{-- Script: slug ģenerēšana (respektē esošu pielāgotu slug) + reveal --}}
    <script>
    (function(){
        ...
        autoEl.checked = (initialSlug === '' || initialSlug === generated);

        if (autoEl.checked && !initialSlug) {
